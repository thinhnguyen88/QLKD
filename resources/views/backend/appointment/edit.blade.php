...
@section ('title', 'Quản lí lịch hẹn | Sửa lịch hẹn')

@section('page-header')
    <h1>
        Quản lí lịch hẹn
        <small>Sửa lịch hẹn <strong>{!! $appointment->title !!}</strong> </small>
    </h1>
@endsection
...
@section('content')
    <form class="form-horizontal" method="post" action="{!! route('admin.appointment.submit.edit' , ['id' =>  $appointment->id]) !!}">
        <input type="hidden" name="_token" value="{!! csrf_token() !!}" />
        <div class="box box-success">
            <div class="box-header with-border">
                <h3 class="box-title">Sửa mới lịch hẹn</h3>
                ...
                <div class="container">
                    ...
                    <div class="col-lg-6">
                            ...
                            <div class="col-lg-9" >
                                <select name="company" id="" class="form-control">
                                    ...
                                    @foreach($list_company as $item)
                                        <option value="{!! $item->id !!}" @if($item->id == $appointment->company_id) selected @endif>{!! $item->company_name !!}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-lg-9" >
                                <input type="text" name="title" required="required" class="form-control" placeholder="Tiêu đề cuộc hẹn" value="{!! old('place' ,$appointment->title ) !!}">
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-lg-3 control-label">Địa điểm</label>
                            <div class="col-lg-9" >
                                <input type="text" name="place" required="required" class="form-control" placeholder="Địa điểm hẹn" value="{!! old('place' , $appointment->place) !!}">
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-lg-3 control-label">Bắt đầu</label>
                            <div class="col-lg-9" >
                                <input type="text" id="start_time" name="start_time" required="required" class="form-control" placeholder="Thời gian bắt đầu " value="{!! old('start_time' , date('d-m-Y H:i', strtotime($appointment->start_time))) !!}">
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-lg-3 control-label">Kết thúc</label>
                            <div class="col-lg-9" >
                                <input type="text" id="finish_time" name="finish_time" required="required" class="form-control" placeholder="Thời gian kết thúc" value="{!! old('finish_time', date('d-m-Y H:i', strtotime($appointment->finish_time))) !!}">
                            </div>
                        </div>
